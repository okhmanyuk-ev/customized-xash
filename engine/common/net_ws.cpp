...
static bool NET_GetWebSocketPacket(netsrc_t sock, netadr_t* from, byte* data, size_t* length);
static void NET_SendWebsocket(netsrc_t sock, int net_socket, const char* buf, size_t len, int flags, const struct sockaddr_storage* to, size_t tolen, size_t splitsize);
...
#define WEBSOCKET_OK 1337

_inline qboolean NET_IsSocketValid( int socket )
{
#if XASH_WEBSOCKET
	return socket == WEBSOCKET_OK;
#else
#if XASH_WIN32 || XASH_DOS4GW
...
{
	const netadr_t *a = (netadr_t*)_a, *b = (netadr_t*)_b;
...
{
#ifdef XASH_WEBSOCKET
	return NET_GetWebSocketPacket(sock, from, data, length);
#else
	...
	*length = 0;
	...
	for( protocol = 0; protocol < 2; protocol++ )
	{
		...
		ret = recvfrom( net_socket, (char*)buf, sizeof( buf ), 0, (sockaddr*)&addr, (socklen_t*)&addr_len);
...
{
#ifdef XASH_WEBSOCKET
	NET_SendWebsocket(sock, net_socket, buf, len, flags, to, tolen, splitsize);
	return len;
#else
...
void NET_SendPacketEx( netsrc_t sock, size_t length, const void *data, netadr_t to, size_t splitsize )
{
	...
	ret = NET_SendLong( sock, net_socket, (const char*)data, length, 0, &addr, NET_SockAddrLen( &addr ), splitsize );
...
class WebsocketClient : public Shared::NetworkingWS::Client
{
public:
	using Shared::NetworkingWS::Client::Client;

public:
	std::list<sky::BitBuffer> mIncomingMessages;

protected:
	std::shared_ptr<Shared::NetworkingWS::Channel> createChannel() override
	{
		auto channel = std::make_shared<Shared::NetworkingWS::SimpleChannel>();
		channel->addMessageReader("msg", [this](sky::BitBuffer& msg) {
...
#ifndef EMSCRIPTEN
static int gServerChannelsCount = 1;
static std::unordered_map<int, void*> gServerChannels;

class ServerChannel : public Shared::NetworkingWS::SimpleChannel
{
public:
	ServerChannel() : SimpleChannel()
	{
		index = gServerChannelsCount;
		gServerChannelsCount += 1;
		assert(!gServerChannels.contains(index));
		gServerChannels.insert({ index, this });
...
class WebsocketServer : public Shared::NetworkingWS::Server
{
public:
	using Shared::NetworkingWS::Server::Server;
...
public:
	...
	std::list<IncomingMsg> mIncomingMessages;

protected:
	std::shared_ptr<Shared::NetworkingWS::Channel> createChannel() override
	{
		auto channel = std::make_shared<ServerChannel>();
		channel->addMessageReader("msg", [this, channel = std::weak_ptr(channel)](sky::BitBuffer& msg) {
			IncomingMsg in_msg;
			in_msg.channel_index = channel.lock()->index;
			in_msg.buf = msg;
			mIncomingMessages.push_back(in_msg);
		});
		return channel;
	}
};
...
static std::optional<WebsocketClient> gWebsocketClient;
#ifndef EMSCRIPTEN
...
qboolean NET_IsClientWebsocketConnected(void)
{
	...
	return gWebsocketClient.value().isConnected();
}

static int NET_CreateWebSocketClient(const std::string& server_address)
{
	try
	{
		assert(!gWebsocketClient.has_value());
#ifdef SKY_USE_OPENSSL
		gWebsocketClient.emplace("wss://" + server_address);
#else
		gWebsocketClient.emplace("ws://" + server_address);
#endif
	}
...
static int NET_CreateWebSocketServer(uint16_t port)
{
#ifndef EMSCRIPTEN
	assert(!gWebsocketServer.has_value());
	try
	{
		gWebsocketServer.emplace(port);
...
static bool NET_GetWebSocketPacket(netsrc_t sock, netadr_t* from, byte* data, size_t* length)
{
	if (sock == NS_CLIENT)
	{
		...
		auto& msgs = gWebsocketClient.value().mIncomingMessages;

		if (msgs.empty())
			return false;

		auto& msg = msgs.front();
		*length = msg.getRemaining();
	...
	{
#ifndef EMSCRIPTEN
		if (!gWebsocketServer.has_value())
			return false;

		auto& msgs = gWebsocketServer.value().mIncomingMessages;

		if (msgs.empty())
			return false;

		auto& msg = msgs.front();
		*length = msg.buf.getRemaining();
		msg.buf.read(data, *length);

		from->ip4 = msg.channel_index;
		from->port = 1337;
		from->type = NA_IP;
...
static void NET_SendWebsocket(netsrc_t sock, int net_socket, const char* buf, size_t len, int flags, const struct sockaddr_storage* to, size_t tolen, size_t splitsize)
{
	if (sock == NS_CLIENT)
	{
		...
		if (!gWebsocketClient.value().isConnected())
			return;

		auto bitbuf = sky::BitBuffer();
		bitbuf.write((void*)buf, len);
		gWebsocketClient.value().getChannel()->sendReliable("msg", bitbuf);
	}
	else if (sock == NS_SERVER)
	{
#ifndef EMSCRIPTEN
		...
		const auto& channels = gWebsocketServer.value().getChannels();

		for (const auto& [index, ptr] : gServerChannels)
		{
			if (adr.ip4 != index)
				continue;

			auto server_channel = (ServerChannel*)ptr;
			auto hdl = server_channel->getHdl();
			channels.at(hdl)->sendReliable("msg", bitbuf);
			return;
		}
#endif
	}
...
static int NET_IPSocket( const char *net_iface, int port, int family )
{
	...
	if( NET_IsSocketError( ioctlsocket( net_socket, FIONBIO, (u_long*)(void*)&_true )))
	{
...
static void NET_OpenIP( qboolean change_port, int *sockets, const char *net_iface, int hostport, int clientport, int family,
	std::optional<std::string> server_addr)
{
	int port;
...
#ifdef XASH_WEBSOCKET
		sockets[NS_SERVER] = NET_CreateWebSocketServer(port);
#else
...
#ifdef XASH_WEBSOCKET
		sockets[NS_CLIENT] = NET_CreateWebSocketClient(server_addr.value());
#else
...
void NET_GetLocalAddress( void )
{
	...
	if( net.allow_ip )
	{
		...
		if( NET_StringToAdrEx( buff, &net_local, AF_INET ))
		{
			...
			if( !NET_IsSocketError( getsockname( net.ip_sockets[NS_SERVER], (sockaddr*)&address, (socklen_t*)&namelen )))
			{
	...
	if( net.allow_ip6 )
	{
		...
		if( NET_StringToAdrEx( buff, &net6_local, AF_INET6 ))
		{
			...
			if( !NET_IsSocketError( getsockname( net.ip6_sockets[NS_SERVER], (sockaddr*)&address, (socklen_t*)&namelen )))
			{
...
static void NET_Config( qboolean multiplayer, qboolean changeport, std::optional<std::string> server_addr)
{
	...
	if( multiplayer )
	{
		// open sockets
#ifdef XASH_WEBSOCKET
		NET_OpenIP(changeport, net.ip_sockets, net_ipname.string, net_iphostport.value, net_ipclientport.value, AF_INET, server_addr);
#else
		if( net.allow_ip )
			NET_OpenIP( changeport, net.ip_sockets, net_ipname.string, net_iphostport.value, net_ipclientport.value, AF_INET, std::nullopt );

		if( net.allow_ip6 )
			NET_OpenIP( changeport, net.ip6_sockets, net_ip6name.string, net_ip6hostport.value, net_ip6clientport.value, AF_INET6, std::nullopt );
#endif
...
{
#ifndef EMSCRIPTEN
	return gWebsocketServer.value().getPort();
#else
	return 0;
#endif
}
...
void HTTP_Run( void )
{
	...
	for( curfile = http.first_file; curfile; curfile = curfile->next )
	{
		...
		if( curfile->state < HTTP_SOCKET ) // Socket is not created
		{
			...
			mode = 1;
			ioctlsocket( curfile->socket, FIONBIO, (u_long*)(void*)&mode );
#if XASH_LINUX
